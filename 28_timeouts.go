...
// TimeoutsDemo - demonstrates timeouts in Go with channels and `select`
func TimeoutsDemo() {
	...
	// `res := <-c1` awaits the result, and `<-time.After(1 * time.Second)`
	// awaits a value to be sent after the timeout of 1s.
	// Since `select` proceeds with the first receive that's ready,
	// we'll take the timeout case if the operation takes more than 1s.
	select {
	case res := <-c1:
		fmt.Println(res)
