...
// StatefulGoroutinesDemo - demonstrates maintaining state with goroutines
func StatefulGoroutinesDemo() {
	...
	// This is the goroutine that owns the state, which is a map
	// private to the stateful goroutine. This goroutine repeatedly
	// selects on the `reads` and `writes` channels, responding to
	// requests as they arrive. A response is executed by first
	// performing the requested operation, and then sending a value
	// on the response channel `resp` to indicate success (and the
	...
	// Read and capture the op counts
	...
	// You should use whichever approach feels most natural, especially with
